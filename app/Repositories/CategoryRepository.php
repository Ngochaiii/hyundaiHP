...
class CategoryRepository extends AbstractRepository {
    ...
    public function readHomeProductCategory() {
        return $this->model->where('type', \App\Models\Category::TYPE_PRODUCT)->where('parent_id', \App\Models\Category::PRODUCT_SHOP_ID)->get();
    }

    public function readHomeGalleryCategory($limit = 10) {
        return $this->model->where('type', \App\Models\Category::TYPE_GALLERY)->where('parent_id', \App\Models\Category::GALLERY_ALBUM)->take($limit)->get();
    }
    ...
    public function readVideoCategory() {
        return $this->model->where('type', \App\Models\Category::TYPE_VIDEO)->where('parent_id', '!=', 0)->orderBy('ordering','asc')->get();
    }

    public function readHomeNewsCategory($limit = 10) {
        return $this->model->where('type', \App\Models\Category::TYPE_NEWS)->where('is_home', 1)->take($limit)->get();
    }
