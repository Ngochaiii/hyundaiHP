...
use App\Repositories\ReviewPersonRepository;
use Mail;
...
class FrontendController extends Controller {
    ...
    //
    public function __construct(SubscriberRepository $subscriberRepo, ProductRepository $productRepo, NewsRepository $newsRepo, GalleryRepository $galleryRepo, TagRepository $tagRepo, PostHistoryRepository $postHistoryRepo, ContactRepository $contactRepo, ReviewPersonRepository $reviewPersonRepo) {
...
//        }
    ...
    public function upload(Request $request){
        ...
        if(!empty($errorUpload)){
            $statusMsg = "Upload Error: " . $errorUpload;
        } elseif(!empty($errorUploadType)){
            $statusMsg = "File Type Error: " . $errorUploadType;
        } else {
            $statusMsg = 'Files are uploaded successfully.';
        }

        return response()->json(array('success' => true, 'message' => $statusMsg, 'image' => $images, 'name' => $name));
    }
    ...
    public function uploadImage(Request $request){
        ...
        if(strlen($fileName) != 0){
            ...
            // Kiểm tra xem loại tệp có hợp lệ không
            if(in_array($fileType, $allowTypes)){
                // Upload file to server
